...
- engine build (NEW) — the fork made runnable per ruling P4: the container
  lane tonight, the nix graduation lane once the patch set freezes; the build
  receipt records which lane served.
- container (NEW) — the image flashnext:dev built by container/build.sh from
  the fork at stable vendor wheels, with a bind-mounted cached build
  directory for incremental iteration.
...
| P4 | engine substrate | the container lane serves the night: stable-index torch 2.13.0+rocm7.14.0 wheels — the fork's exact pin, published on the vendor stable channel — with the fork built from source in an image whose build directory is bind-mounted with compiler caching, so a one-line patch iteration recompiles only what changed; the nix derivation is the graduation lane, prepared tonight as wiring plus audit but never tonight's critical path — a moving patch set is not a thing to nixify, and the nix substrate charges a full recompile for every one-line change; the build receipt names the lane |
...
| P7 | transport | socket transport on both rails is the overnight transport of record, both rails named to the collective library; the RDMA package is prepared overnight but brought up only operator-attended, on one rail only, and only after a committed socket-transport benchmark is banked — the kernel half is entirely unpaid on this fleet, the worker's deploy path must move off the fast rail before any kernel change, the reboot order is worker first and never both at once, and adoption comes only on a counterbalanced win |
...
3.1 the engine build lands through the ruling P4 container lane → a build receipt records status pass with the lane, the resolved torch and triton versions, and the wall-clock. [gate: receipts-verify]
...
UNKNOWN-6 the wall-clock of the first full container engine build on this machine — measured by the build receipt; iteration after it rides the cached build directory.
UNKNOWN-7 whether the pinned packaging expression satisfies every override signature and literal against the fork tree — the graduation-lane question, drained by the substrate-compat audit; it blocks nothing tonight.
